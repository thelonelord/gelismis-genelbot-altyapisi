  ...
  try {
    if (!x) {
      const e = new Discord.MessageEmbed()
        .setColor("RANDOM")
        .setDescription(
          `<:yildirim:789739751624278027>  Rozeti: **50 ELMAS** (Satın Almak İçin: **${prefix}market rozet**)`
        );
    ...
    if (x === "rozet") {
      if (rozet > 1) {
        const embed = new Discord.MessageEmbed()
          .setDescription(
            `<a:neoncarpi:780444956849340416> Zaten sende rozet bulunuyor fazlasını ne yapacaksın?`
          )
          .setColor("RED")
          .setTimestamp();
        ...
        message.channel.send(
          `<a:neoncarpi:780444956849340416> Rozet satın almak için yeterli elmasın bulunuyor. \n Gerekli olan elmas: **${rozetF} ELMAS**.`
        );
      } else if (rozetA > rozetF) {
        message.channel.send(
          `${client.emojis.cache.get(
            client.emojiler.paraROZET
          )} Rozetini başarıyla aldınız`
        );
        db.set(
          `memberBadge6_${message.author.id}`,
    ...
    if (x === "kalem") {
      if (kalem > 0) {
        const embed = new Discord.MessageEmbed()
          .setDescription(
            `<a:neoncarpi:780444956849340416> Zaten sende kalem eldiveni bulunuyor fazlasını ne yapacaksın?`
          )
          .setColor("RED")
          .setTimestamp();
        message.channel.send({ embed });
        ...
        message.channel.send(
          `<a:neoncarpi:780444956849340416> Kalem satın almak için yeterli paran bulunuyor. \n Gerekli olan para: **250TL**.`
        );
      } else if (para > 200) {
        message.channel.send(
          `<a:yesil_onay:727045346852601908> :pencil2: Kalem başarıyla aldınız`
        );
