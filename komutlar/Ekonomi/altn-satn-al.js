...
exports.run = async (client, message, args) => {
  ...
  if (miktar.match(x)) return message.reply("Matematik işlemi girmiyeceksin!");
  ...
  if (para < altinpara * miktar) {
    message.channel.send(
      `:no_entry: ${miktar} Altın alabilmek için yeterli paran görünmüyor. \n Gerekli para **${miktar *
        altinpara} **`
    );
    ...
    const aldıE2 = new Discord.MessageEmbed()
      ...
      .setDescription(
        ...
        )} Başarıyla marketten **${miktar}** altın aldınız\n Hesabınızdan eksilen para miktarı: **${miktar *
          altinpara}TL**`
      )
      .setTimestamp();
