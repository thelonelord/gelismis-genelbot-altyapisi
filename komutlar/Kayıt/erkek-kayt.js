...
exports.run = async (client, message, args) => {
  ...
  //    let prefix = await r${message.guild.id}`) || ayarlar.prefix
  //   const ayarlar = require('../ayarlar.json')
  let prefix =
  ...
  if (!message.member.roles.cache.has(yetkili)) {
    const hata = new Discord.MessageEmbed()
      ...
      .setDescription(`Bu komut için yetkili rolüne sahip olmalısınız!`)
      .setColor("RED");
    //  .setTimestamp()
  ...
  if (!kisi) {
    const hata = new Discord.MessageEmbed()
      //  .setAuthor('HATA', message.author.avatarURL())
      .setDescription(
        `Lütfen bir kullanıcıyı etiketleyin!\n\n**Örnek Kullanım:** \n\`\`\`${prefix}erkek-kayıt <@kullanıcı> İsim Yaş\`\`\` `
      )
      .setColor("RED");
    //   .setTimestamp()
    return message.channel.send(hata);
  ...
  if (!isim) {
    const hata = new Discord.MessageEmbed()
      //    .setAuthor('HATA', message.author.avatarURL())
      .setDescription(
        `Bir isim girmelisin. Kullanıcın iki ismi varsa lütfen bir tanesini giriniz!\n\n**Örnek Kullanım:** \n\`\`\`${prefix}erkek-kayıt <@kullanıcı  İsim Yaş\`\`\` `
      )
      .setColor("RED");
    //    .setTimestamp()
    return message.channel.send(hata);
  ...
  if (isim.length > 12) {
    const hata = new Discord.MessageEmbed()
      //    .setAuthor('HATA', message.author.avatarURL())
      .setDescription(
        `Lütfen doğru bir isim giriniz! Girdiğiniz isim çok uzun!\n\n**Örnek Kullanım:** \n\`\`\`${prefix}erkek-kayıt <@kullanıcı> İsim Yaş\`\`\` `
      )
      .setColor("RED");
    //   .setTimestamp()
    return message.channel.send(hata);
  ...
  if (yaş.length > 2) {
    const hata = new Discord.MessageEmbed()
      //  .setAuthor('HATA', message.author.avatarURL())
      .setDescription(
        `Lütfen doğru bir yaş giriniz! Girdiğiniz yaş çok büyük!\n\n**Örnek Kullanım:** \n\`\`\`${prefix}e @kullanıcı İsim Yaş\`\`\` `
      )
      .setColor("RED");
    //    .setTimestamp()
    return message.channel.send(hata);
  ...
  const embed22 = new Discord.MessageEmbed()
    //   .setTitle(`Mükemmel!`)
    .setDescription(
      `**${kisi} Kullanıcısına <@&${mutel}> Rolü Verildi!**
  ...
  const yar = new Discord.MessageEmbed().setTitle(`İsim Kayıt Log - #${num}`)
    .setDescription(`
