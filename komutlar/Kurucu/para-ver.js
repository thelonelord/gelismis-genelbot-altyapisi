...
exports.run = async (client, message, args) => {
  ...
  if (user.bot === true)
    return message.reply("Parayı bir bota yollarsan sen zararlı çıkarsın!");
  ...
  if (isNaN(args[1]))
    return message.channel.send(
      "Bir yazımı yollayacaksın? Lütfen bir miktar gir!."
    );
  ...
  const embed = new Discord.MessageEmbed()
    .setColor("RANDOM")
    .setDescription(
      `<:emoji_50:742746450194333778>  Başarıyla ${user} adlı kullanıcıya para yollandı \n Yollanılan miktar: ${mesaj}`
    );
