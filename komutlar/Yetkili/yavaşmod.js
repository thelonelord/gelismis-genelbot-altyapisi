...
exports.run = async (client, msg, args) => {
  ...
  if (!limit) {
    var embed = new Discord.MessageEmbed()
      .setDescription(
        `Yazma sınırını (süresini) kaç saniye olarak ayarlamak istediğinizi yazınız!`
      )
      .setColor("RANDOM");
  ...
  if (limit > 10) {
    var x = new Discord.MessageEmbed()
      .setDescription(
        "Yazma sınırı (süresi) limiti maksimum **10** saniye olabilir!"
      )
      .setColor("RANDOM");
  ...
  var e = new Discord.MessageEmbed()
    .setDescription(
      `Yazma sınırı (süresi) başarıyla **${limit}** saniye olarak ayarlanmıştır!`
    )
    .setColor("RANDOM");
