...
const yes = ["evet", "kabul ediyorum", "ediyorum", "evt", "yes"];
const no = ["hayır", "kabul etmiyorum", "etmiyorum", "hyr", "no"];
...
class ek {
  ...
  static duration(ms) {
    ...
    return `${hrs.padStart(2, "0")}:${min.padStart(2, "0")}:${sec.padStart(
      2,
      "0"
    )}`;
  }
  ...
  static async randomFromImgurAlbum(album) {
    ...
    return body.data.images[Math.floor(Math.random() * body.data.images.length)]
      .link;
  }
